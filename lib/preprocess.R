...
# post.monthly <- df %>% mutate(month=format(df$ARREST_DATE,"%Y-%m")) %>%
#   filter(ARREST_DATE>=as.Date("2020-3-1")) %>% 
#   group_by(month) %>% 
#   tally()
# post.monthly.max <- post.monthly[which(post.monthly$n==max(post.monthly$n)),]
# pre.monthly <- df %>% mutate(month=format(df$ARREST_DATE,"%Y-%m")) %>%
#   filter(ARREST_DATE<as.Date("2020-3-1")) %>% 
#   group_by(month) %>% 
#   tally()
# pre.monthly.max <- pre.monthly[which(pre.monthly$n==max(pre.monthly$n)),]
